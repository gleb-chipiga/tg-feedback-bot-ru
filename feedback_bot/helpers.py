...
async def send_from_message(bot: Bot, chat_id: int, from_chat: Chat) -> None:
    await bot.send_message(
        chat_id, f"От {user_link(from_chat)}", parse_mode=ParseMode.HTML
    )
...
class AlbumForwarder:
    ...
    async def _send(
        ...
        while True:
            ...
            if message.audio is not None:
                ...
                media.append(
                    InputMediaPhoto(
                        media=max(
                            message.photo, key=attrgetter("file_size")
                        ).file_id,
                        caption=message.caption,
                        caption_entities=message.caption_entities,
                    )
        ...
        if len(media) > 0:
            ...
            await self._bot.send_message(
                from_chat.id,
                "Не удалось переслать элементов "
                f"неподдерживаемого типа: {message_count}",
            )
            logger.debug(
                "Failed to forward %d media group items of "
                "unsupported type",
                message_count,
            )
        else:
